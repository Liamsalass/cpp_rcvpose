...
Trainer::Trainer(const Options& opts, DenseFCNResNet152& model)
{
    ...
    if (!opts.resume_train) {
        ...
        try {
            ...
            // Load optim and other data 
            CheckpointLoader loader(out, true, false);
            epoch = loader.getEpoch();
            starting_epoch = epoch;
            if (opts.verbose) {
                cout << "Epoch: " << epoch << endl;
            }
            optim = loader.getOptimizer(model);
    ...
    while (epoch < max_epoch) {
        ...
        if (((epoch % 10) == 0) && (epoch != 0)) {
            ...
            for (const auto& batch : *val_loader) {
                ...
                cv::Mat img_mat = torch_tensor_to_cv_mat(img);
                cv::Mat rad_1_mat = torch_tensor_to_cv_mat(rad_1);
                cv::Mat rad_2_mat = torch_tensor_to_cv_mat(rad_2);
                cv::Mat rad_3_mat = torch_tensor_to_cv_mat(rad_3);
                cv::Mat sem_target_mat = torch_tensor_to_cv_mat(sem_target);
                cv::imshow("Image", img_mat);
                cv::imshow("Radial 1", rad_1_mat);
                cv::imshow("Radial 2", rad_2_mat);
                cv::imshow("Radial 3", rad_3_mat);
                cv::imshow("Semantic", sem_target_mat);
                cv::waitKey(100);
