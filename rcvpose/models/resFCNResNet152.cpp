...
ResFCNResNet152Impl::ResFCNResNet152Impl(int input_channels, int output_channels) :
	...
	// Encoders
	conv1(nn::Conv2dOptions(input_channels, 64, 7).stride(2).padding(3).bias(false)),

	bn1(64),

	relu(nn::ReLUOptions().inplace(true)),

	maxpool(nn::MaxPool2dOptions(3).stride(2).padding(1)),

	block1up(64, 64, 1, true),

	block1(nn::Sequential(
		Bottleneck(256, 64, 1), Bottleneck(256, 64, 1), Bottleneck(256, 64, 1))
	),

	block2up(256, 128, 2, true),

	block2(nn::Sequential(Bottleneck(512, 128, 1), Bottleneck(512, 128, 1),
	...
	block3(nn::Sequential(Bottleneck(1024, 256, 1), Bottleneck(1024, 256, 1),
	...
	block4(nn::Sequential(
		Bottleneck(2048, 512, 1), Bottleneck(2048, 512, 1), Bottleneck(2048, 512, 1))
	),

	// Decoders
	conv_up4(nn::Sequential(
		nn::Conv2d(nn::Conv2dOptions(2048, 1024, 3).stride(1).padding(1)),
		nn::BatchNorm2d(1024),
		nn::ReLU(true)
	)),

	conv_up4_1(nn::Conv2dOptions(1024, 2048, 1)),

	up4(nn::UpsampleOptions().scale_factor(std::vector<double>({ 2 })).mode(torch::kBilinear).align_corners(false)),

	conv_up3(nn::Sequential(
		nn::Conv2d(nn::Conv2dOptions(1024, 512, 3).stride(1).padding(1)),
		nn::BatchNorm2d(512),
		nn::ReLU(true)
	)),

	conv_up3_1(nn::Conv2dOptions(512, 1024, 1)),

	up3(nn::UpsampleOptions().scale_factor(std::vector<double>({ 2 })).mode(torch::kBilinear).align_corners(false)),

	conv_up2(nn::Sequential(
		nn::Conv2d(nn::Conv2dOptions(512, 256, 3).stride(1).padding(1)),
		nn::BatchNorm2d(256),
		nn::ReLU(true)
	)),

	conv_up2_1(nn::Conv2dOptions(256, 512, 1)),

	up2(nn::UpsampleOptions().scale_factor(std::vector<double>({ 2 })).mode(torch::kBilinear).align_corners(false)),
	...
	conv_up1_1(nn::Conv2dOptions(64, 256, 1)),

	up1(nn::UpsampleOptions().scale_factor(std::vector<double>({ 2 })).mode(torch::kBilinear).align_corners(false)),
	...
	conv8(nn::Conv2dOptions(64, output_channels, 1).stride(1))
{
